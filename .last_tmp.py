...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='PEJUANG TANGGUH=>', color='[CC]'):
    ...
    def run(self):
        try:
            ...
            self.log('BERUSAHA AJA DULU PASTI ADA HASILNYA '.format(self.proxy_host, self.proxy_port))
            ...
            self.log('JANGAN PANTANG MENYERAH  ''[CC]'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
...
def main():
    print(colors('\n'.join([
     'JIKA BERUSAHA PASTI AKAN ADA HASIL','[CC]'
     'TRIK BARU UNTUK TELKOMSEL OPOK','[CC]'
     'CHANNEL YT: NURHASNIA SALAM','[CC]'
     'SEMOGA BERHASIL ','[CC]'
    ])))    
