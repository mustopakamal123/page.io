...
def log(value, status='[ tumbal janda dulu ]', color='[CC]'):
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('[R1] sabar anjing'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('[G1] Alhamdulillah''[CC]'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
...
def main():
    print(colors('\n'.join([
     ...
     '[P1] ------    MUSTOPA KAMAL    ------','[CC]'
     '[G1] ==================================','[CC]'
     '[P1] ------  Gunakan YG BENAR ----- ','[CC]'
     '[G1] ==================================','[CC]'
     '[G1] HOST : 127.0.0.1 ','[CC]'
     '[G1] PORT : 8080 ','[CC]'
     '[R1] MODE SABAR ',' [CC]'
     '[Y1] SUNEK  ON/OFF DATA ','[CC]'
     '[Y1]    MODE PESAWAT  ','[CC]'
     '[R1]   ','[CC]'
